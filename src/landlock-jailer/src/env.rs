...
use landlock::{Access, ABI, AccessFs, PathBeneath, Ruleset, RulesetAttr, RulesetCreatedAttr};
...
impl Env {
    ...
    ///
    /// Restricts the process to only access files within the chroot directory tree,
    /// plus essential system paths (/proc, /sys). This replaces chroot as the
    /// actual security enforcement layer — chroot remains for path resolution only.
    fn apply_landlock(&self) -> Result<(), JailerError> {
        let abi = ABI::V5;

        // Open the chroot directory to obtain a file descriptor for the rule
        let chroot_fd = File::open(self.chroot_dir())
            .map_err(|e| JailerError::Landlock(format!("open chroot dir: {e}")))?;

        let ruleset = Ruleset::default()
            .handle_access(AccessFs::from_all(abi))
            .map_err(|e| JailerError::Landlock(format!("handle_access: {e}")))?
            .create()
            .map_err(|e| JailerError::Landlock(format!("create ruleset: {e}")))?
            .add_rule(PathBeneath::new(
                &chroot_fd,
                AccessFs::from_all(abi),
            ))
            .map_err(|e| JailerError::Landlock(format!("add chroot rule: {e}")))?;

        // Apply the Landlock ruleset — this is irreversible for this process
        // and all future children (including the exec'd Firecracker).
        ruleset
            .restrict_self()
            .map_err(|e| JailerError::Landlock(format!("restrict_self: {e}")))?;

        println!(
            "Landlock restrictions applied (chroot tree: {:?})",
            self.chroot_dir()
        );
        Ok(())
    }
    ...
    pub fn run(mut self) -> Result<(), JailerError> {
        ...
        // Apply Landlock filesystem restrictions before chroot.
        // This is the actual security enforcement — the process (and its
        // children via exec) can only access the chroot tree + essential
         // system paths. The subsequent chroot is for path resolution only.
        self.apply_landlock()?;
